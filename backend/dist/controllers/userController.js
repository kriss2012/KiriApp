...
export const getEmployabilityScore = async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        if (user.githubUrl) {
            ...
            if (username) {
                try {
                    const headers = { 'User-Agent': 'ASG-Community-App' };
                    if (process.env.GITHUB_TOKEN) {
                        headers['Authorization'] = `token ${process.env.GITHUB_TOKEN}`;
                    }
        ...
        if (user.githubUrl) {
            ...
            if (gitStars > 0)
                githubActivityScore += 20;
            githubActivityScore = Math.min(100, githubActivityScore);
        }
        // Weighted sum
        const overallScore = Math.round((skillScore * 0.30) +
            (projectScore * 0.20) +
            (resumeScore * 0.15) +
            (certScore * 0.10) +
            (mockScore * 0.10) +
